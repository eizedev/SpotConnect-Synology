#!/usr/bin/env python3
"""
Validate that SpotConnect binaries are well-formed ELF executables for the
Synology architecture they are being packaged for, and report their
minimum-kernel and glibc-version requirements. Linux/ELF only - SpotConnect
...
This exists because AirConnect-Synology (this project's sibling, same build
pipeline) shipped a release in its issue #107 where
...
Usage:
    ...
    validate_elf.py --arch arm target/spotupnp target/spotraop
    validate_elf.py --arch x86_64-static target/spotupnp target/spotraop \
...
# EM_* constants from <elf.h>, limited to what SpotConnect actually ships.
EM_MACHINE_NAMES = {
    3: "x86",
    8: "mips",
    20: "powerpc",
...
def parse_elf(data):
    ...
    - that failure mode is deliberate: a corrupted binary (like the one in
    AirConnect-Synology's issue #107)
