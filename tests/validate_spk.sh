#!/bin/sh
# Validate the structure of a built SpotConnect .spk package: required
...
check_one() {
    ...
    if [ -d "$workdir/scripts" ]; then
        # The exact set this package ships. There is deliberately no
        # postuninst: unlike AirConnect-Synology, this package creates no
        # shared folder, so nothing survives the package directory being
        # removed and there is nothing left to clean up afterwards.
        for script in preinst postinst preuninst preupgrade postupgrade start-stop-status; do
